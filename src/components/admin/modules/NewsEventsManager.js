...
function NewsEventsManager() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const data = await newsEventsApi.getAll(token);
      setItems(data.data || []);
      setError(null);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingItem?._id) {
        await newsEventsApi.update(token, editingItem._id, payload);
      } else {
        await newsEventsApi.create(token, payload);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await newsEventsApi.delete(token, id);
      await fetchItems();
    } catch (err) {
      setError(err.message);
    }
              ...
              items.map((item) => (
                <tr key={item._id} className="hover:bg-[#f8f5f0]">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-[#3a5c40]">{typeof item.title === 'string' ? item.title : (item.title?.en || '')}</div>
