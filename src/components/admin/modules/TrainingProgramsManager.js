...
function TrainingProgramsManager() {
  ...
  const resetForm = () => {
    setFormData({
      title: '', description: '', type: 'Online', duration: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingProgram
        ? `${API_BASE_URL}/api/admin/training-programs/${editingProgram._id}`
        : `${API_BASE_URL}/api/admin/training-programs`;
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error?.message || 'Failed to save training program');
      }
      ...
      {showForm && (
        <div className="bg-white rounded-lg shadow-md p-6 border border-[#d6c7b0]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[#5a4a3a] mb-1">Description *</label>
              <textarea
              ...
              filteredPrograms.map((program) => (
                <tr key={program._id} className="hover:bg-[#f8f5f0]">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-[#3a5c40]">{program.title}</div>
