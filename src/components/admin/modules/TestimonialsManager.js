...
function TestimonialsManager() {
  ...
  const resetForm = () => {
    setFormData({
      customerName: '', customerLocation: '', treatment: '',
      testimonialText: '', rating: 5, imageUrl: '', isApproved: false,
      dateReceived: new Date().toISOString().split('T')[0]
    });
    setFormErrors({});
    setEditingItem(null);
  ...
  const validateForm = () => {
    ...
    if (!formData.testimonialText.trim()) errors.testimonialText = 'Testimonial text is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingItem
        ? `${API_BASE_URL}/api/admin/testimonials/${editingItem._id}`
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error?.message || 'Failed to save testimonial');
      }
  ...
  const handleApprove = async (id, currentStatus) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/testimonials/${id}/approve`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border border-[#d6c7b0]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[#5a4a3a] mb-1">Treatment *</label>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-[#5a4a3a] mb-1">Testimonial Text *</label>
              <textarea
        ...
        {testimonials.length === 0 ? (
          ...
          testimonials.map((item) => (
            <div key={item._id} className="bg-white rounded-lg shadow-md p-6 border border-[#d6c7b0]">
              ...
              <p className="text-sm text-[#6b5344] mb-2">Treatment: {item.treatment}</p>
              <p className="text-[#5a4a3a] text-sm line-clamp-3 mb-4">{item.testimonialText}</p>
