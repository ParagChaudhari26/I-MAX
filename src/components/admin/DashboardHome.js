...
function DashboardHome() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const [programsRes, newsRes, testimonialsRes, blogsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/admin/training-programs`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${API_BASE_URL}/api/admin/news-events`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
      ...
      const [programs, news, testimonials, blogs] = await Promise.all([
        programsRes.ok ? programsRes.json() : { data: [] },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border border-[#d6c7b0]">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Link
              to="/admin/dashboard/training-programs"
              className="flex items-center p-3 bg-[#f0e8d8] rounded-lg hover:bg-[#e8dcc8] transition-colors"
            >
              <svg className="w-5 h-5 text-[#3a5c40] mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
              <span className="text-sm text-[#3a5c40] font-medium">New Program</span>
            </Link>
            <Link
            ...
            >
              ...
              <span className="text-sm text-[#3a5c40] font-medium">New Testimonial</span>
            </Link>
          </div>
        </div>
      </div>
